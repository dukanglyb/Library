...
class JsonControllerBase extends Base
{
    ...
    public function   cutPic(&$psotArray, $fileName, $required = true)
    {
        ...
        if ($_FILES && array_key_exists($fileName, $_FILES)) {
            ...
            $ret1 = $this->CurlFileRestService->performPostRequest("/v1/tfs", file_get_contents($array['src']));
    ...
    public function uploadFile(&$psotArray,$fileNameArray,$required = true){
        ...
        foreach($fileNameArray as $fileName){
            if($_FILES&&array_key_exists($fileName,$_FILES)) {
                $ret1 = $this->CurlFileRestService->performPostRequest("/v1/tfs", file_get_contents($_FILES[$fileName]["tmp_name"]));
                //TODO 添加服务器连接失败提示
                $psotArray[$fileName] = json_decode($ret1)->TFS_FILE_NAME;
                //记录已经上传了这个文件
                ...
                foreach($uploadedFileNames as $uploadedFileName){
                    $this->CurlFileRestService->performDeleteRequest('/v1/tfs/'.$uploadedFileName,null);
                }
                return false;
            }
        }
    ...
    public function deleteFile(&$psotArray,$fileNameArray,$data){
        ...
        foreach($fileNameArray as $fileName){
            if($_FILES&&array_key_exists($fileName,$_FILES)&&json_decode($data)->zhuangTaiMa<0){
                $this->CurlFileRestService->performDeleteRequest('/v1/tfs/'.$psotArray[$fileName],null);
